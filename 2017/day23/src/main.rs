...
impl Inst {
    fn new (s: &str) -> Inst {
        lazy_static! {
            static ref TWO_ARG_RE: Regex = Regex::new(r"^(set|sub|mul) ([a-z]) (-?[a-z0-9]+)$").unwrap();
            static ref JNZ_ARG_RE: Regex = Regex::new(r"^jnz (-?[a-z0-9]+) (-?[a-z0-9]+)$").unwrap();
        }
        if let Some(cap) = TWO_ARG_RE.captures(s) {
            ...
            Inst::Jnz(Val::new(cap.get(1).unwrap().as_str()),
