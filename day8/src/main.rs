...
impl Instr {
    fn new(t_ : String) -> Instr {
        lazy_static! {
            ...
            static ref ROT_ROW_RE : Regex = Regex::new("^rotate row y=([0-9]+) by ([0-9]+)$").unwrap();
            static ref ROT_COL_RE : Regex = Regex::new("^rotate column x=([0-9]+) by ([0-9]+)$").unwrap();
        }
        let mut ret : Instr = Instr::Rect(0,0);
        if RECT_RE.is_match(&t_) {
            for cap in RECT_RE.captures_iter(&t_) {
                let ref width_ = cap.at(1);
                let ref height_ = cap.at(2);
                let width : usize = width_.unwrap().parse().unwrap();
                let height : usize = height_.unwrap().parse().unwrap();
                ret = Instr::Rect(width, height);
            }
        } else if ROT_ROW_RE.is_match(&t_) {
            for cap in ROT_ROW_RE.captures_iter(&t_) {
                let ref y_ = cap.at(1);
                let ref by_ = cap.at(2);
                let y : usize = y_.unwrap().parse().unwrap();
                let by : usize = by_.unwrap().parse().unwrap();
...
impl Screen {
    ...
    fn count (&self) -> usize {
        self.pixels.into_iter().fold(
            0,
            |aa, &p| if p { aa + 1 } else { aa }
        )
    }
