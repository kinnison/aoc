    ...
    fn new (t_ : String) -> Instr {
        ...
        if CPY_I_RE.is_match(&t_) {
            for cap in CPY_I_RE.captures_iter(&t_) {
                let ref val_ = cap.at(1);
                let ref reg_ = cap.at(2);
                let reg = reg_from(reg_.unwrap());
                let val : i32 = val_.unwrap().parse().unwrap();
                return Instr::CpyI(val, reg);
            }
        } else if CPY_R_RE.is_match(&t_) {
            for cap in CPY_R_RE.captures_iter(&t_) {
                let ref regi_ = cap.at(1);
                let ref rego_ = cap.at(2);
                let regi = reg_from(regi_.unwrap());
                let rego = reg_from(rego_.unwrap());
                return Instr::CpyR(regi, rego);
            }
        } else if INC_RE.is_match(&t_) {
            for cap in INC_RE.captures_iter(&t_) {
                let ref reg_ = cap.at(1);
                let reg = reg_from(reg_.unwrap());
            ...
            for cap in JNZ_I_RE.captures_iter(&t_) {
                ...
                let ref val_ = cap.at(2);
                let val : i32 = val_.unwrap().parse().unwrap();
                let tst : i32 = tst_.unwrap().parse().unwrap();
                if (tst != 0) {
...
impl VM {
    ...
    fn exec_instr (&mut self, i : &Instr) -> bool {
        ...
        match *i {
            Instr::CpyI(val, reg) => { self.set_reg(reg, val); },
    ...
    fn run_program (&mut self, prog : Vec<Instr>) {
        loop {
            ...
            if self.pc >= prog.len() { panic!("Fell way off!"); }
        }
    }
}
