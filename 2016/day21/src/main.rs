...
impl Instr {
    fn new (t_: String) -> Instr {
        lazy_static! {
            static ref SWAP_POS_RE:  Regex = Regex::new("swap position ([0-9]+) with position ([0-9]+)").unwrap();
            ...
            static ref ROT_LETTR_RE: Regex = Regex::new("rotate based on position of letter (.)").unwrap();
        ...
        if let Some(cap) = SWAP_LET_RE.captures(&t_) {
            let ref x_ = cap.at(1);
            let ref y_ = cap.at(2);
            let x: char = x_.unwrap().chars().nth(0).unwrap();
            let y: char = y_.unwrap().chars().nth(0).unwrap();
            return Instr::SwapLetter(x as u8, y as u8);
        }
        if let Some(cap) = ROT_LEFT_RE.captures(&t_) {
            let ref s_ = cap.at(1);
            let steps: usize = s_.unwrap().parse().unwrap();
        ...
        if let Some(cap) = ROT_LETTR_RE.captures(&t_) {
            let ref l_ = cap.at(1);
            let letter: char = l_.unwrap().chars().nth(0).unwrap();
